...
The budget is 500ms against a measured worst case of 7 to 12ms, and the measurement is what took it there. Two full pipes are all a reaped child can leave behind, which is 131072 bytes on macOS, and draining both took 7.3 to 11.6ms across four runs. The first value was 100ms, roughly 9x that, which is thin for a CI runner an order of magnitude slower than the machine it was measured on. Widening it is nearly free: nothing joins the pump task, and `Msg::Exited` has already gone by the time the pump ends, so no operator waits on this budget and `shep stop` does not return any later for it.
