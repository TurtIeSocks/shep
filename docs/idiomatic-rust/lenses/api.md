...
- **Config structs**: `AppConfig::builder()` fits the user's builder-pattern rule; validation returns `Result<AppConfig, NormalizeError>` at build, mirroring `Uniform::new -> Result` (`uniform.rs:235`). No panicking constructors in shep-core at all — panicking conveniences belong to shep-cli only.
