...
Do call `.seconds(Seconds::Optional)`. That is the dialect promise: a six-field pattern with a leading seconds field is accepted, and a five-field pattern still means what it always meant. It also means the existing stopgap is not merely incomplete but *wrong in both directions* — it accepts `not a cron five` (five whitespace-separated tokens) and rejects `0 30 2 * * *` (a valid six-field pattern).
...
2. **Is a memory-limit breach measured over the process tree, or the root pid?** This plan measures the tree (sheep plus lambs), which is a deliberate deviation from pm2's single-pid behaviour. The argument is that the tree is what gets killed, so the tree is what should be measured, and a root-pid limit is dodged by any app that forks. The counter-argument is surprise: a user migrating from pm2 with `max_memory = "512M"` on a forking app will see restarts they did not see before. Whichever way this goes, it wants a line in `docs/migration.md`.
3. **Do `https://` probe targets get rejected, or accepted-and-deferred?** This plan rejects them at config time with a typed error, because the hand-rolled HTTP client (decision D1) has no TLS and a probe that silently fails every poll is indistinguishable from an app that is down. The cost is that a user with an HTTPS health endpoint cannot use a readiness probe at all in v1. The alternative is adding a TLS stack, which is the dependency this phase was explicitly designed to avoid.
4. **How wide is the cron dialect, exactly?** croner accepts `L`, `W`, `#`, `?` and an optional seconds field — richer than the five-field cron most users mean by "cron", and richer than what map.md's compat note describes. This plan takes croner's dialect as-is with `Seconds::Optional`, which is the widest reading. Narrowing it later is a breaking change to the Flockfile grammar, so it is worth one deliberate look now.
5. **Does the bench crate land in this phase?** Task 5 builds it because IR-26 requires a benchmark-backed comment on `MEMORY_POLL_INTERVAL` and IR-5 specifies exactly what a bench crate should look like — but it is the phase's only task that ships no user-visible behaviour, and deferring it means the const's comment cites a one-off measurement instead of a committed harness. If the answer is "defer", say so before Task 4 finishes, because Task 6's const comment is written either way.
6. **Does a memory-limit or liveness restart count against `max_restarts`?** This plan says no: both go through `SupervisorHandle::restart`, which resets the budget, and spec §4 counts only exits inside `min_uptime` as unstable. That means an app in a memory-leak loop restarts forever rather than reaching `errored`. The opposite choice needs a second restart path in the engine, which is why it is a question rather than a default.
7. **`MAX_CRON_SLEEP = 60s`, and at-most-one catch-up.** A cron worker re-derives its next occurrence at least once a minute, so a suspended laptop or an NTP step costs at most a minute of drift; and a daemon that missed six occurrences fires once on wake rather than six times. Both are restart-storm guards and both are guesses at what a user wants. Sixty seconds is one wakeup per minute per cron-configured sheep, which is cheap but not free on a large flock.
8. **The watch group restarts every instance of a name.** Spec §4 says one watcher per name-group, and this plan takes the restart to be group-wide to match. A rolling per-instance restart on file change would keep the app serving through a save, which is arguably what a watch is *for* in development — but it is also reload's job, and reload is not in this phase.
