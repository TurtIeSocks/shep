...
- **Unsafe policy:** shep-daemon lib.rs `#![deny(unsafe_code)]`; any unsafe lives ONLY in `sys.rs` behind module-scoped `#[allow(unsafe_code)]` + rationale + per-block `// SAFETY:` (IR-22/23). Target: zero unsafe (command-fds + nix + std cover the needs). — Met for 2a itself (shipped zero unsafe); Phase 2b's Task 7 later needed real fd-adoption unsafe, confined to `sys.rs` — the maintainer's Decision 1 (2026-08-08) restructured `BootOptions::ready_fd` (`Option<std::fs::File>`, adopted by the CALLER) so this holds exactly as originally stated, with zero unsafe in `boot.rs`; see the 2b plan doc's own correction on this same line for the full history.
