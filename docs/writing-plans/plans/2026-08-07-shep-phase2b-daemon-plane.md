...
- **Invoke the `shep-idiomatic-rust` skill before writing any Rust**; cite IR rules. Clean-room: never open `~/GitHub/pm2`.
...
- **Unsafe policy:** shep-daemon lib.rs keeps `#![deny(unsafe_code)]`; **Task 7 spends the phase's only unsafe** — one block in `sys.rs` behind a module-scoped `#[allow(unsafe_code)]`, a per-block `// SAFETY:`, and the IR-24 rationale essay (rejected alternative + why each failure mode can't happen). No other module may contain unsafe. — CORRECTED (Task 7 execution, 2026-08-08): `adopt_fd` had to become an `unsafe fn` itself, not a safe fn hiding one internal block, because its ordering precondition is a CALLER obligation no internal check can verify (see `sys.rs`'s own rationale essay, scenario (c)). That makes `boot.rs` the one OTHER module that does contain unsafe — its single call site into `adopt_fd`, with its own `// SAFETY:` comment. The phase's unsafe surface is two documented sites, each independently justified, not one block confined to `sys.rs` alone; see task-11-report.md's doc-claims table for the full sweep. — RESTORED (the maintainer's Decision 1, 2026-08-08): the prior correction diagnosed a real bug (boot.rs's SAFETY comment claimed a false invariant) but fixed it by widening the RULE instead of the DESIGN. The maintainer rejected that. Structural fix instead: `BootOptions::ready_fd` became `Option<std::fs::File>` — but the CLI's `main` (Phase 3) never populates it: `ready_fd` stays `None` unconditionally, readiness is established by a handshake instead of an inherited descriptor, and the CLI never calls `adopt_fd` or sets `SHEP_READY_FD`. `boot.rs` therefore calls `adopt_fd` not at all and contains zero unsafe, and the adoption path this phase built becomes dead code by design (see phase3-cli.md's own note). The original sentence above ("No other module may contain unsafe") is true again, literally, not just in spirit — see `sys.rs`'s own doc for the final, exhaustive accounting. — SCOPED (Windows tier, 2026-08-28): every claim in this paragraph is about the unix tree this phase built. The later Windows tier added `sys_windows.rs`, which carries that platform's own unsafe under the same per-block `// SAFETY:` rule and its own accounting.
...
> **CORRECTED (the maintainer's Decision 1, 2026-08-08):** the interface block above is the ORIGINAL plan, and it's stale on more than one axis by now (`adopt_fd` shipped as `unsafe fn` not the safe `pub fn` shown above; `signal_ready`/`BootError::Ready` shipped split into `write_ready`/`ReadyWrite`, not the single fn shown above — both already true before this decision). The axis this correction is actually about: `BootOptions.ready_fd` shipped as `Option<RawFd>`, which a later whole-branch review found unsound (a safe caller could build a `BootOptions` that drove `boot`'s internal `unsafe { sys::adopt_fd(fd) }` block into UB with no `unsafe` keyword of its own — the exact gap `sys.rs`'s own `# Safety` section warns about). The fix actually shipped: `ready_fd: Option<std::fs::File>` — but the CLI's `main` (Phase 3) never constructs a `Some`: `ready_fd` stays `None` unconditionally, and readiness is established by a handshake instead of an inherited descriptor. `boot` therefore performs no adoption, and no unsafe, of its own. `sys::adopt_fd` keeps its `unsafe fn` shape (still the right one — the ordering precondition is a caller obligation no internal check can verify) but has no production caller at all today; it and `SHEP_READY_FD` are dead code by design. Net effect: `boot.rs` has zero unsafe, `sys.rs`'s file is once again this crate's entire unsafe surface, and IR-22 as originally written ("All unsafe confined to `shep-daemon/src/sys.rs`") holds literally, not just in spirit. Current shipped shape: `crates/shep-daemon/src/boot.rs` and `crates/shep-daemon/src/sys.rs`, both `//!`-documented.
...
- Exactly one `unsafe` block in the workspace, in `sys.rs`, with a `// SAFETY:` comment and the IR-24 rationale essay. — CORRECTED (Task 7 execution, 2026-08-08): this criterion is stale. `adopt_fd` had to become an `unsafe fn` (not a safe wrapper around one internal block) because its ordering precondition is a CALLER obligation no internal check can verify — see `sys.rs`'s own rationale essay, scenario (c). The workspace's unsafe surface is TWO documented sites, each independently justified: `sys::adopt_fd`'s own definition, and its one call site in `boot::boot` (each with its own `// SAFETY:` comment). Any other doc in this repo still claiming "one unsafe block" is equally stale and should be corrected the same way (Task 11 closed the ones found). — RESTORED (the maintainer's Decision 1, 2026-08-08): the "two documented sites" correction above is itself now stale, by design rather than by drift. `boot::boot`'s call site is gone: `BootOptions::ready_fd` is `Option<std::fs::File>`, but the CLI's `main` (Phase 3) leaves it `None` unconditionally rather than adopting anything — readiness is a handshake instead — so `boot` never touches `adopt_fd` and contains no unsafe. `sys.rs`'s file is once again this crate's entire unsafe surface — the ORIGINAL criterion at the top of this line is true again, not the correction beneath it. See the Task 7 Interfaces block's own correction (above) for the full account.
