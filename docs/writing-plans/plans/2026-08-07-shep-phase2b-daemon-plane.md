...
crates/shep-daemon/src/
  ...
  sys.rs        unsafe fd adoption (2 sites incl. boot.rs's call)  Task 7
...
- Exactly one `unsafe` block in the workspace, in `sys.rs`, with a `// SAFETY:` comment and the IR-24 rationale essay. — CORRECTED (Task 7 execution, 2026-08-08): this criterion is stale. `adopt_fd` had to become an `unsafe fn` (not a safe wrapper around one internal block) because its ordering precondition is a CALLER obligation no internal check can verify — see `sys.rs`'s own rationale essay, scenario (c). The workspace's unsafe surface is TWO documented sites, each independently justified: `sys::adopt_fd`'s own definition, and its one call site in `boot::boot` (each with its own `// SAFETY:` comment). Any other doc in this repo still claiming "one unsafe block" is equally stale and should be corrected the same way (Task 11 closed the ones found).
