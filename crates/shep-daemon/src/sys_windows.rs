...
//! another one is a *nested* job with its own rules, since the outer job's
//! limit flags constrain it and Windows before 8 had no nesting at all.
//!
//! This module carried a reporting surface for a while to find out whether
//! that was behind two tests that passed on a developer's Windows box and
//! hung on `windows-latest`. It was not: the cause was a PowerShell test
//! fixture that never ran, and the runner's own outer job turned out to
//! contain a nested job correctly. The surface is gone with the question,
//! and `tests/real_runner_windows.rs` asserts the containment that matters.

#![allow(unsafe_code)] // IR-24 exception — the Windows twin of `sys.rs`'s, and
// the only one on this platform. Ten sites, all in this file: eight `unsafe`
// blocks plus the two `unsafe impl`s that make a kernel handle `Send` and
// `Sync`. Of the blocks, seven are FFI calls (`CreateJobObjectW`,
// `SetInformationJobObject`, `AssignProcessToJobObject`, `TerminateJobObject`,
// `CloseHandle`, `GetStdHandle` and `SetHandleInformation`) and one is a
// `mem::zeroed` for a Win32 limit struct. Nothing outside this module writes
// `unsafe` on Windows.
